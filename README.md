...
* Assign a spin $\sigma_v\in \{-1,1\}$ with probability $\mathbb{P}_{T,\boldsymbol{\theta}}\left( \sigma_v = 1\right) = \mathbb{P}_{T,\boldsymbol{\theta}}\left(\sigma_v = -1\right) = \frac{1}{2}.$
...
$Z_u = Z_u^{T_u, \widehat{\boldsymbol{\theta}}}(\sigma_{{L_u}}) = \mathbb{E}_{\widehat{\boldsymbol{\theta}}}[\sigma_u | \sigma_{{L_u}}]$ is the expected spin at the vertex $u$ (computed with the parameter ${\widehat{\boldsymbol{\theta}}}$) given the spins at the leaves $L_u$ generated under the true distribution $\sigma\sim \mathbb{P}_{T_u,\boldsymbol{\theta}^*}$. 
...
$\theta_e^*, \widehat{\theta}_e\in \left[1-\frac{1}{2}\delta, 1-\frac{1}{4}\delta\right]\qquad \forall e\in E$
then
$\mathbb{P}_{\boldsymbol{\theta}^*}\left(\sigma_u Z_u^{\widehat{\boldsymbol{\theta}}} \ge 1-\frac{121}{5} \delta^2\right) \ge 1-\frac{7}{2}\delta$ for all trees $T$ and all $\delta\in(0, 1/924]$. 
